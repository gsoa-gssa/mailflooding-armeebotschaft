<x-app-container>
    <div class="flood-form__outer min-h-screen flex items-center">
        <div class="flood-form w-full max-w-[980px] mx-auto px-4 py-12">
            ...
            <h1 class="mb-4 font-machina font-bold text-5xl">{{__("flood.form.title2")}}</h1>
            ...
            <form action="/app/submission/2" method="POST" class="!mt-0">
                <div class="flex justify-end gap-x-2 py-2 sticky top-0 z-50 bg-black bg-opacity-80">
                    <a href="#" id="select-all" class="flood-button !bg-white">{{__("flood.form.select-all")}}</a>
                    <button type="submit" class="flood-button">{{__("flood.form.next")}}</button>
                </div>
                @if ($errors->any())
